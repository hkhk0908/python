...
class MambaModel(BaseModel):
    def __init__(self, vocab_size, input_dim, hidden_dim):
        ...
        self.B = nn.Parameter(torch.rand(hidden_dim, hidden_dim))
        self.C = nn.Parameter(torch.rand(hidden_dim, hidden_dim))
        self.D = nn.Parameter(torch.rand(hidden_dim))

    def forward(self, x):
        u = F.gelu(self.linear_u(x)) * torch.sigmoid(self.linear_g(x))
        s = torch.zeros(x.size(0), x.size(2), device=x.device)
        outputs = []
...
train_data = batchify(data_process(torchtext.datasets.WikiText2(split='train')), batch_size)
...
df.set_index("Model")[["Latency (ms)", "Memory (MB)"]].plot(kind='bar', figsize=(10, 5),
    title="모델별 실행시간 및 메모리 사용량")
plt.grid(True)
plt.tight_layout()
plt.show()
